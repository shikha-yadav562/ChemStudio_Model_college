...
class _WetTestAFinalResultScreenState extends State<WetTestAFinalResultScreen> {
  bool isLoading = true;
  bool _showRecheck = false; // ← toggle for recheck panel

  Map<int, GroupStatus> studentGroups = {};
  List<String> selectedIons = [];
  Map<int, String?> _studentCTAnswers = {}; // ← for recheck
  ...
  Future<void> _loadData() async {
    ...
    for (final ids in _groupCTIds.values) {
      for (final id in ids) {
        ctAnswers[id] = await db.getStudentAnswer('SaltA_WetTest', id);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView( // ← changed from Padding+Column to allow scroll when recheck opens
        ...
        child: Column(
          children: [
            ...
            // ── RECHECK PANEL (expands when button pressed) ──
            if (_showRecheck) ...[
