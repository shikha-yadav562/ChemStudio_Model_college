...
class WetTestBFinalResultScreen extends StatefulWidget {
  final String salt;
  const WetTestBFinalResultScreen({super.key, required this.salt});

  @override
  State<WetTestBFinalResultScreen> createState() =>
      _WetTestBFinalResultScreenState();
}

class _WetTestBFinalResultScreenState extends State<WetTestBFinalResultScreen> {
  bool isLoading = true;
  ...
  Future<void> _loadData() async {
    ...
    for (final ids in _groupCTIds.values) {
      for (final id in ids) {
        ctAnswers[id] = await db.getStudentAnswer('SaltB_WetTest', id);
      }
    }
